...
export class AuthController {
    ...
    try {
      ...
      if (idToken) {
        ...
        try {
          ...
          const decodedToken = await this.authService.verifyFirebaseTokenOnly(
            idToken
          );
          ...
          const user = await this.authService.createUserProfile(
            decodedToken.uid,
            {
              email,
              first_name,
      ...
      if (body.preferences) {
        updates.preferences = {
          ...
          notification_settings: body.preferences.notification_settings && {
            ...
            digest_frequency:
              body.preferences.notification_settings.digest_frequency ??
              "daily",
          },
  ...
  getUsersByStatus = async (req: Request, res: Response) => {
    try {
      ...
      if (
        !status ||
        !["active", "inactive", "pending"].includes(status as string)
      ) {
        res.status(400).json({
          status: "error",
          code: "INVALID_STATUS",
